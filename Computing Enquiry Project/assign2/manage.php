...
if (isset($_POST['delete_submit'])) {
    ...
    if (mysqli_stmt_affected_rows($stmt) > 0) {
        ...
        // Redirect to the same page to refresh the display table
...
if (isset($_POST["eoiNumber"]) || isset($_POST["job_reference"]) || isset($_POST["first_name"]) || isset($_POST["last_name"])) {
    ...
    if ($first_name != "") {
        if ($condition != "")
            $condition .= " and first_name LIKE '%$first_name%'";
        else
            $condition .= "first_name LIKE '%$first_name%'";
    }
    if ($last_name != "") {
        if ($condition != "")
            $condition .= " and last_name LIKE '%$last_name%'";
        else
            $condition .= "last_name LIKE '%$last_name%'";
    }
